...
**Note:** The MCP server automatically checks br CLI version >= 0.9.0 on startup. This command provides detailed version info and update instructions.

Use the beads MCP tools to:
1. Run `br version` via bash to get the CLI version
2. Check the plugin version (0.9.2)
3. Compare versions and report any mismatches

Display:
- br CLI version (from `br version`)
...
- Update br CLI: `cargo install beads_rust` or see https://github.com/Dicklesworthstone/beads_rust
